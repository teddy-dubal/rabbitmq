...
class RpcCallback implements ProcessorInterface
{
    ...
    public function process(Message $message, array $options): bool
    {
        ...
        if (class_exists($action[0]) && method_exists($action[0], $action[1])) {
            ...
            $this->_dic['log']->debug(sprintf('[RPC] action %s in %s => %s', $body['client.call_action'], strval($time), $this->msg));
        } else {
            $this->_dic['log']->error(sprintf('[RPC] unkknow callback %s', $body['client.call_action']));
        }
        return true;
    }
